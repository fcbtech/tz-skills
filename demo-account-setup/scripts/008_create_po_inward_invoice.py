"""Create a demo Purchase Order, full-quantity Inward, then a Purchase Invoice.
...
first supplier in the company's network, picks the first two buyable
goods, and creates:
  ...
  2. an Inward against that PO marking 100% of the ordered quantity as
     received;
  3. a Purchase Invoice from that PO covering the same lines.

All three documents share the same transaction chain.
"""
...
def _build_po_item(
    *,
    ...
    total_amount = round(total_cost + tax_amount, 2)
    return {
        "id": product["id"],
        "product": product["id"],
...
def _rehydrate_item(ds_item: dict, tax_master: dict, products_by_id: dict) -> dict:
    """Copy a doc_structure item and apply transforms required for the create payload.

    - Strip inventory-level `custom_fields` (psycopg2 rejects nested dict).
    - Replace rich `taxes` array with the minimal create-shape entry from the tax master.
    - Reset `taxes_data` to {} (server recomputes).
    - Transform `unit` (str -> object) and `units` (int -> array) from the products map.
    """
    item = dict(ds_item)
...
def discover_context(token: str) -> dict[str, Any]:
    ...
    if not suppliers:
        sys.exit("No suppliers available in counter-party list — add a supplier counter-party first.")
...
def create_po(token: str, ctx: dict, qty_by_product: dict[int, int]) -> dict:
    ...
    items = [
        _build_po_item(
            product=product,
            quantity=qty_by_product[product["id"]],
            tax_master=ctx["gst"],
            self_id=self_id,
            delivery_date_str=today,
            position=pos,
        )
        for pos, product in enumerate(ctx["products"])
    ]
...
def create_inward(token: str, ctx: dict, po_view: dict, qty_by_product: dict[int, int]) -> dict:
    self_id = ctx["self_company_id"]
    sup_id = ctx["supplier_id"]
    ...
    for pos, raw in enumerate(ds_items):
        ...
        product_id = item.get("product") or item.get("id")
        full_qty = qty_by_product[product_id]
        item["quantity"] = str(full_qty)
        item["product_delivered_now"] = str(full_qty)
...
def create_invoice(token: str, ctx: dict, po_view: dict) -> dict:
    ...
    products = {p["id"]: p for p in (dd.get("products") or [])}
    if len(ds_items) != NUM_ITEMS:
        sys.exit(f"Invoice doc_structure should carry {NUM_ITEMS} items from PO; got {len(ds_items)}")
    ...
    payload = {
        ...
        "primaryDocumentDetails": {
            ...
            # (psycopg2 TypeError). The invoice is still tied to the inward via the shared
            # transaction chain.
            "select_inward": [],
            "store_details": ctx["store"],
            "exportDetails": EXPORT_DETAILS,
...
def main() -> None:
    ...
    qty_by_product = {p["id"]: random.randint(QTY_MIN, QTY_MAX) for p in ctx["products"]}
